...
flags.DEFINE_integer('max_steps', 1000, 'Number of steps to run trainer.') #8 epoch de 138.25 steps
  ...
  with tf.Graph().as_default():
    ...
    with tf.variable_scope('var_name') as var_scope:
      weights = {
              ...
              'mlpw1': _variable_with_weight_decay('mlpw1', [4096, 2048], 0.0005),
              'mlpw2': _variable_with_weight_decay('mlpw2', [2048, 256], 0.0005),
              'out': _variable_with_weight_decay('wout', [256, c3d_model.NUM_CLASSES], 0.0005)
              }
      biases = {
              ...
              'mlpb1': _variable_with_weight_decay('mlpb1', [2048], 0.0000),
              'mlpb2': _variable_with_weight_decay('mlpb2', [256], 0.0000),
    ...
    for step in range(FLAGS.max_steps):
      ...
      if (step) % 10 == 0 or (step + 1) == FLAGS.max_steps:
        saver.save(sess, os.path.join(model_save_dir, 'c3d_rvl_model.ckpt'), global_step=step)
        print('Training Data Eval:')
        summary, acc, l = sess.run(
        ...
        print ("accuracy: " + "{:.5f}".format(acc))
        val_data=[acc, l]
        with open('val_data.csv', 'a', newline='') as f:
                            writer = csv.writer(f)
