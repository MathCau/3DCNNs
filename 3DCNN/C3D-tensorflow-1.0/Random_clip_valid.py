...
class C3dModel(object):
    def __init__(self,
        ...
        # train clip: 9537*5 CLIP=5 (2212 pour moi car 79x14x2)
        # test  clip: 3783*5 CLIP=5
        # train clip: 9537*3 CLIP=3
        # test  clip: 3783*3 CLIP=3
        self.n_step_epoch=int( 2212/batch_size)
    ...
    def test(self, test_list, modelpath):
        with self.graph.as_default():
            c3d_net = [
                ...
                ["reshape", [-1, 8192]],
                ["fc", "fc1", [8192, 8192], 'wd1', 'bd1', True],
                ["h", "h1", [8192, 2048], 'h1', 'b1'],
                ["h", "h2", [2048, 256], 'h2', 'b2'],
            ...
            with tf.Session(config=config, graph=self.graph) as sess:
                ...
                saver.restore(sess, modelpath + "c3d_ucf_model-119")
                ...
                for one_epoch in range(1):
                    ...
                    total_v = 0.0
                    test_correct_num = 0
                    for i in tqdm(range(int(1106 / self.batch_size))):
                        step += 1
...
if __name__ == "__main__":
    c3dnet = C3dModel()
    c3dnet.test(test_list="./list/tests.list",
                 modelpath="./models")
