...
class C3dModel(object):
    def __init__(self,
        ...
        # train clip: 9537*5 CLIP=5 (620 test clips)
        # test  clip: 3783*5 CLIP=5
        # train clip: 9537*3 CLIP=3
        # test  clip: 3783*3 CLIP=3
        self.n_step_epoch=int(2248/batch_size)
    ...
    def layer_mlp(self, inputs, shape, name,w_name,b_name,activation = True):
        with self.graph.as_default():
            with tf.variable_scope('var_name') as var_scope:
                W = tf.get_variable(name = w_name, shape = shape, initializer = self.initializer, dtype = tf.float32)
                b = tf.get_variable(name = b_name, shape = shape[-1], initializer = tf.zeros_initializer(), dtype = tf.float32)
    ...
    def test(self, test_list, modelpath, model_name):
        with self.graph.as_default():
            c3d_net = [
                ...
                ["layer_mlp", "layer_1", [4096, 2048], 'mlpw1', 'mlpb1', True],
                ["layer_mlp", "layer_2", [2048, 256], 'mlpw2', 'mlpb2', True],
                ["fc", "fc3", [256, self.num_class],'wout','bout',False],
            ]
            ...
            with tf.Session(config=config, graph=self.graph) as sess:
                ...
                train_var_list = [v for v in tf.trainable_variables() if v.name.find("c1") == -1]
                ...
                #train_op = tf.train.MomentumOptimizer(self.lr,0.9).minimize(
                     #total_loss, global_step = 110)


                total_para = np.sum([np.prod(v.get_shape().as_list()) for v in train_var_list])
                print('total_para:', total_para)  # all CDC9 :28613120  #pool5 27655936

                # train clip:2248
                # test  clip: 620
                init = tf.global_variables_initializer()
                var_list = [v for v in tf.trainable_variables() if v.name.find("c1") != -1]  # 初始化只加载卷积层参数
                print(var_list)
                ...
                for one_epoch in range(1):
                    ...
                    total_v = 0.0
                    test_correct_num = 0
                    for i in tqdm(range(int(2248/self.batch_size))):
                        step += 1
                        ...
                        print('test acc:', test_correct_num / total_v, 'test_correct_num:', test_correct_num,
                              'total_v:', total_v,'total_loss:', total_loss)

if __name__ == "__main__":
    c3dnet = C3dModel()
    c3dnet.test(test_list="list/train.list",
                 modelpath='models/',
                 model_name = "models/c3d_rvl_model.ckpt-199")
